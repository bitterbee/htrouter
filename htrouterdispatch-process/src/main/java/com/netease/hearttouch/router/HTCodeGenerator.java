...
final class HTCodeGenerator {
    ...
    public static TypeSpec generateManagerClass(String packageName, List<HTAnnotatedClass> classes) {
        ...
        FieldSpec mTargetUrlExtraKey = FieldSpec
                .builder(String.class, "HT_TARGET_URL_KEY", PUBLIC, STATIC, FINAL)
                .initializer("\"HTROUTER_TARGET_URL_KEY\"")
                .build();
        builder.addField(mTargetUrlExtraKey);
        ...
        MethodSpec.Builder isUrlRegisteredMethod = MethodSpec.methodBuilder("isUrlRegistered")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .addParameter(String.class, "url")
                .returns(boolean.class)
                .beginControlFlow("if (TextUtils.isEmpty(url))")
                .addStatement("return false")
                .endControlFlow()
                .addStatement("return findRouterEntryByUrl(url) != null");
        isUrlRegisteredMethod.addJavadoc("根据传入的 url 能否找到相关的跳转页面信息" +
                "@param url 需要进行匹配的URL\n" +
                "@return 能找到相关的跳转页面信息，返回true，否则返回false\n");
        builder.addMethod(isUrlRegisteredMethod.build());

        MethodSpec.Builder startActivityWithCustomAnimMethod = MethodSpec.methodBuilder("startActivity")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .returns(void.class)
                ...
                .addParameter(int.class, "entryAnim")
                .addParameter(int.class, "exitAnim")
                ...
                .beginControlFlow("if (exitAnim == 0 && entryAnim == 0)")
                ...
                .endControlFlow()
                .beginControlFlow("if (entryAnim != 0 || exitAnim != 0)")
                ...
                .endControlFlow()
                .endControlFlow();
        startActivityWithCustomAnimMethod.addJavadoc("通过URL启动一个页面\n" +
        ...
        builder.addMethod(startActivityWithCustomAnimMethod.build());

        MethodSpec.Builder startActivityWithAnimMethod = MethodSpec.methodBuilder("startActivity")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .returns(void.class)
                .addParameter(ClassName.get("android.app", "Activity"), "activity")
                .addParameter(String.class, "url")
                .addParameter(ClassName.get("android.content", "Intent"), "sourceIntent")
                .addParameter(boolean.class, "isFinish")
                .addStatement("$N(activity, url, sourceIntent, isFinish, 0, 0)", startActivityWithCustomAnimMethod.build());
        startActivityWithAnimMethod.addJavadoc("通过URL启动一个页面\n" +
                "@param activity 当前需要进行跳转的activity\n" +
                "@param url 跳转的目标URL\n" +
                "@param sourceIntent 传递进来一个intent，用户数据及启动模式等扩展\n" +
                "@param isFinish 跳转后是否需要关闭当前页面\n");
        builder.addMethod(startActivityWithAnimMethod.build());

        MethodSpec.Builder startActivityContextWithAnimMethod = MethodSpec.methodBuilder("startActivity")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .returns(void.class)
                ...
                .addParameter(int.class, "entryAnim")
                .addParameter(int.class, "exitAnim")
                .beginControlFlow("if (context instanceof $T)", ClassName.get("android.app", "Activity"))
                .addStatement("$N(($T)context, url, sourceIntent, isFinish, entryAnim, exitAnim)", startActivityWithCustomAnimMethod.build(), ClassName.get("android.app", "Activity"))
                .addStatement("return")
                .endControlFlow()
                .addStatement("$T intent = null", ClassName.get("android.content", "Intent"))
                .addStatement("$T routerParams = new $T(url, sourceIntent)",
                        ClassName.get(packageName, "HTRouterHandlerParams"),
                        ClassName.get(packageName, "HTRouterHandlerParams"))
                .beginControlFlow("if (sHtRouterHandler != null && sHtRouterHandler.handleRoute(context, routerParams))")
                .addStatement("return")
                .endControlFlow()
                .addStatement("url = routerParams.url")
                .addStatement("sourceIntent = routerParams.sourceIntent")
                .addStatement("HTRouterEntry entry = getRouterEntry(url)")
                .beginControlFlow("if(entry != null)")
                .addStatement("intent = processIntent(context, sourceIntent, url, entry.getActivity())")
                .addStatement("intent.addFlags($T.FLAG_ACTIVITY_NEW_TASK)", ClassName.get("android.content", "Intent"))
                .addStatement("context.startActivity(intent)")
                .nextControlFlow("else if(mWebActivityClass != null)")
                .addStatement("intent = processIntent(context, sourceIntent, url, mWebActivityClass)")
                .addStatement("intent.addFlags($T.FLAG_ACTIVITY_NEW_TASK)", ClassName.get("android.content", "Intent"))
                .addStatement("intent.putExtra(mWebExtraKey,getHttpSchemaHostAndPath(url))")
                .addStatement("context.startActivity(intent)")
                .endControlFlow();
        startActivityContextWithAnimMethod.addJavadoc("通过URL启动一个页面\n" +
                ...
                "@param entryAnim 自定义的入场动画\n" +
                "@param exitAnim 自定义的出场动画\n");
        builder.addMethod(startActivityContextWithAnimMethod.build());

        MethodSpec.Builder startActivityContextMethod = MethodSpec.methodBuilder("startActivity")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .returns(void.class)
                .addParameter(ClassName.get("android.content", "Context"), "context")
                .addParameter(String.class, "url")
                .addParameter(ClassName.get("android.content", "Intent"), "sourceIntent")
                .addParameter(boolean.class, "isFinish")
                .addStatement("$N(context, url, sourceIntent, isFinish, 0, 0)", startActivityContextWithAnimMethod.build());
        startActivityContextMethod.addJavadoc("通过URL启动一个页面\n" +
                "@param context 当前需要进行跳转的 context\n" +
                "如果 context 是 Activity，则使用 startActivity(Activity activity, String url, Intent sourceIntent, boolean isFinish)进行跳转\n" +
                "否则这个函数进行处理，但参数 isFinish 不会生效，另外启动页面的场景切换动画会使用主题默认动画\n" +
                "@param url 跳转的目标URL\n" +
                "@param sourceIntent 传递进来一个intent，用户数据及启动模式等扩展\n" +
                "@param isFinish 跳转后是否需要关闭当前页面\n");
        builder.addMethod(startActivityContextMethod.build());

        MethodSpec.Builder startActivityForResultWithAnimMethod = MethodSpec.methodBuilder("startActivityForResult")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .returns(void.class)
                .addParameter(ClassName.get("android.app", "Activity"), "activity")
                .addParameter(String.class, "url")
                ...
                .endControlFlow()
                .endControlFlow();
        startActivityForResultWithAnimMethod.addJavadoc("通过URL启动一个页面,同时可以获得result回调\n" +
                "@param activity 当前需要进行跳转的activity\n" +
                "@param url 跳转的目标URL\n" +
                "@param sourceIntent 传递进来一个intent，用户数据及启动模式等扩展\n" +
                "@param isFinish 跳转后是否需要关闭当前页面\n" +
                "@param entryAnim 自定义的入场动画\n" +
                "@param exitAnim 自定义的出场动画\n");
        builder.addMethod(startActivityForResultWithAnimMethod.build());
        ...
        MethodSpec.Builder startActivityForResultMethod = MethodSpec.methodBuilder("startActivityForResult")
                ...
                .addParameter(boolean.class, "isFinish")
                .addParameter(int.class, "requestCode")
                .addStatement("$N(activity, url, sourceIntent, isFinish, requestCode, 0, 0)", startActivityForResultWithAnimMethod.build());
        ...
        MethodSpec.Builder startActivityForResultFragmentWithAnimMethod = MethodSpec.methodBuilder("startActivityForResult")
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC)
                .returns(void.class)
                ...
                .addParameter(int.class, "entryAnim")
                .addParameter(int.class, "exitAnim")
                .beginControlFlow("if (fragment.getActivity() == null)")
                .addStatement("return")
                .endControlFlow()
                .addStatement("$T intent = null", ClassName.get("android.content", "Intent"))
                .addStatement("$T routerParams = new $T(url, sourceIntent)",
                ...
                .endControlFlow()
                .endControlFlow();
        startActivityForResultFragmentWithAnimMethod.addJavadoc("通过URL启动一个页面,同时可以获得result回调，回调在fragment中\n" +
                ...
                "@param entryAnim 自定义的入场动画\n" +
                "@param exitAnim 自定义的出场动画\n");
        builder.addMethod(startActivityForResultFragmentWithAnimMethod.build());
        ...
        MethodSpec.Builder startActivityForResultFragmentMethod = MethodSpec.methodBuilder("startActivityForResult")
                ...
                .addParameter(boolean.class, "isFinish")
                .addParameter(int.class, "requestCode")
                .addStatement("$N(fragment, url, sourceIntent, isFinish, requestCode, 0, 0)", startActivityForResultFragmentWithAnimMethod.build());
        ...
        MethodSpec.Builder processIntentMethod = MethodSpec.methodBuilder("processIntent")
                ...
                .endControlFlow()
                .addStatement("intent.putExtra(HT_TARGET_URL_KEY, Uri.parse(url));")
